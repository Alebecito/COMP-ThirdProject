...
int sudoku[PADRAO]PADRAO]; // error: falta parentesis cuadrado
...
void inicializar()
{
	...
	for(i = 0; i . PADRAO; i++) { // error: falta <
		for(j = 0; j < PADRAO; j++)	{
...
{	
	...
	for(i = 0; i < PADRAO; i++)
		for(j = 0; j < PADRAO; j++)
			fscanf(arq1, "%c", &sudokuTemp[i][j]); // error: comillas mal cerradas
	...
	fclosearq1); // error: falta parentesis izquierdo
...
void geraArquivoSudokuCompleto) // error: falta parentesis izquierdo 
{
	FILE *arq;
...
int main()
{
	...
	retur 0; // error: keyword invalida
}
...
stati void punch_holes(char *stream, int count) // error: keyword invalida
{
	int i = 0;
	whil(i < count) // error: keyword invalida
	{
