...
int invaders_init(struct invaders *in int score, int lives); // falta ,
...
static int update_invader(struct invadernode *p, void *data)
{
    ...
    if (in->ishot != NULL)
        return 0;
    if (rand() / (double)RAND_MAX >= length(p->next) / (double)length(in->invader))
...
int invaders(struct invaders *in, int move, int shoot)
{
    ...
    freelist in->artifacts); //falta parentesis izquierdo
    in->artifacts = NULL;
    ...
    if (in->countdown <= 0)
    {
        ...
        else if ((in->dtn < 0 && find(in->invader, &firstcol, on_column)) || (in->dtn > 0 && find(in->invader, &lastcol, on_column)))
            in->dtn = 0;
...
struct invadernode *new (int 5v, int x, int y, int size, struct invadernode *next) // identificador invalido
{
...
int input(int *move, int *shoot, int *pause)
{
    ...
    *shoot = *move = 0;
    while ((r = tb_peek_event(&ev, 0))  0) // falta operador (relacional, aritmetico, etc)
    {
        if (r ! TB_EVENT_KEY) // ! es invalido
            continue;
...
static void set(int x-sa, int y, int c, int fg, int bg) // caracter invalido en identificador, por lo que es inesperado
{
    int dx, dy;
