...
static int update_invader(struct invadernode *p, void *data)
{
	...
	if (in->dtn == 0)
		...
		p->x += in->dtn;
	while ((q = find(in->barrier, p, collision)) != 0)
		delete(&in->barrier, q);
	if (in->ishot != 0)
		return 0;
	if (rand() / (double) RAND_MAX
	  >= length(p->next) / (double) length(in->invader))
		in->ishot = new(rand() % 2, p->x, p->y + 1, 1, 0);
	return 0;
}
...
int invaders(struct invaders *in, int move, int shoot)
{
	...
	in->artifacts = 0;
	if (shoot && (in->sshot == 0 || in->sshot->y < INVADERS_HEIGHT/2))
		in->sshot = new(0, in->ship->x + 1, in->ship->y, 1, in->sshot);
	if (move < 0 && in->ship->x > 0)
		in->ship->x--;
	if (move > 0 && in->ship->x < INVADERS_WIDTH - 1)
		in->ship->x++;
	map(&in->special, 0, update_special);
	if (in->special == 0 && rand() % 300 == 0)
		in->special = new(0, INVADERS_WIDTH - 1, 0, 3, 0);
	if (in->countdown <= 0) {
	...
	if (find(in->invader, &lastrow, on_row))
		in->lives = 0;
	return (in->lives > 0 && in->invader != 0);
}
...
int invaders_init(struct invaders *in, int score, int lives)
{
	...
	in->dtn = +1;
	in->artifacts = 0;
	in->special = in->sshot = in->ishot = 0;
	in->ship = new(0, 0, 13, 3, 0);
	in->invader = init_invaders();
	in->barrier = init_barriers();
	if (in->ship == 0 || in->invader == 0 || in->barrier == 0) {
		free(in->ship);
		freelist(in->invader);
		freelist(in->barrier);
		return -1;
...
void delete(struct invadernode **listp, struct invadernode *target)
{
	...
	if (*listp == target)
		*listp = target->next;
	else for (p = *listp; p->next != 0; p = p->next)
		if (p->next == target) {
			p->next = target->next;
			break;
		}
...
static void drawgame(struct invaders *in)
{
	...
	for (p = in->ship; p != 0; p = p->next)
		for (i = 0; i < p->size; i++)
			set(p->x + i, p->y, "=^"[i == p->size/2], TB_RED, 0);
	for (p = in->invader; p != 0; p = p->next)
		for (i = 0; i < p->size; i++)
			set(p->x + i, p->y, ":o@"[p->v], 0, 0);
			flag++;
	for (p = in->barrier; p != 0; p = p->next)
		for (i = 0; i < p->size; i++)
			set(p->x + i, p->y, "# "[p->v], TB_WHITE, TB_RED);
			flag++;
	for (p = in->special; p != 0; p = p->next) {
		set(p->x, p->y, '<', TB_RED, 0);
		for (i = 1; i < p->size-1; i++)
			set(p->x + i, p->y, '=', TB_RED, 0);
		set(p->x + p->size - 1, p->y, '>', TB_RED, 0);
	}
	for (p = in->ishot; p != 0; p = p->next)
		set(p->x, p->y, "!$"[p->v], 0, 0);
	for (p = in->sshot; p != 0; p = p->next)
		set(p->x, p->y, '|', 0, 0);
	for (p = in->artifacts; p != 0; p = p->next)
		for (i = 0; i < p->size; i++)
