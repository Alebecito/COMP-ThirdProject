...
typedef struct vector
{
        ...
        int (*at)(const struct vector*, int);
        void (*reserve)(struct vector*, int);
        int (*capacity)(const struct vector*);
        void (*set)(struct vector*, int, int);
        void (*assign)(struct vector*, int, int);
...
{
        void* (*current)(void*);
        void (*set_time)(struct get_time*, int*, int*, int, int); 
...
typedef struct piece
{
        ...
        void (*loop)(struct piece*, void (*lhs)(struct piece*, int, int), void (*rhs)(struct piece*, int), int);
...
int vector_at(const vector*, int n);
...
void vector_assign(vector*, int, int);
...
void get_time_set_time(get_time*, int*, int*, int, int);
...
void piece_loop(piece*, void (*lhs)(piece*, int, int), void (*rhs)(piece*, int), int);
...
int vector_at(const vector* this, int n)
{
    return this->vec[n];
}

void vector_set(vector* this, int i, int k)
{
     this->vec[i] = k;
}
...
void vector_reserve(vector* this, int n)
{
     this->v_capacity = n;
     int* new_vec = malloc(n*sizeof(int));
     int s = n > this->v_size ? this->v_size : n;
     memcpy(new_vec, this->vec, s*sizeof(int));
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->vec);
     this->vec = new_vec;
}

void vector_resize(vector* this, int n)
{
     this->reserve(this, n);
     this->v_size = n;
}

void vector_assign(vector* this, int len, int k)
{
...
int vector_capacity(const vector* this)
{
...
max_distance* max_distance_max_vertical_coordinate(max_distance* this)
{
     ...
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(col);
     return this;
}
...
void transpose_unique(transpose* this, vector* vec)
{
     ...
     for (int i = 0; i != vec->size(vec); i += 2)
     {
          int p = vec->at(vec, i);
...
{
     for (int i = 0; i != new_box->size(new_box); i += 2)
     {
...
pair transpose_mid_point(vector* mid)
{
     int size = mid->size(mid);
     int len = size/2-(size%4)/2;
...
{
     int len = this->new_box.size(&this->new_box);
     this->new_cdt.assign(&this->new_cdt, len, 0);
     for (int i = 0; i != len; i += 2)                                                          
     {                                                                                          
          this->new_cdt.set(&this->new_cdt, i, this->new_box.at(&this->new_box, i+1)+m-n);      
          this->new_cdt.set(&this->new_cdt, i+1, (this->new_box.at(&this->new_box, i)-m)*dx+n); 
     }                                                                                          
     if ( dx == 0.5 ) 
...
void get_time_set_time(get_time* this, int* lhs, int* rhs, int value, int i)
{
     if ( *lhs == value )
     {
          ++*rhs;
...
void* get_time_current(void* ptr)
{
      ...
      (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(line);
      ...
      for (int i = 0; i != 4; ++i)
           (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->time[i]);
      (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->time);
      return NULL;
}
...
void piece_loop(piece* this, void (*lhs)(piece*, int, int), void (*rhs)(piece*, int), int call)
{
...
void piece_get_preview(piece* this, vector* box0, string old_block, int n, string new_block_color)
{
     ...
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(buffer);
...
void piece_coordinate(piece* this, vector* lbox)
{
     ...
     char* buffer = malloc(str_size);
     for (int i = 0; i != locus.size(&locus); i += 2)
     {
...
void piece_random_piece(piece* this)
{
     this->count++;
     int k = height-1;
     for (int i = 0, j = 6; i != this->count; j += 2)
     {
          if ( j == wthm ) { j = 4; ++i; }
          if ( rand()%2 ) 
          { 
...
void piece_top_point(piece* this, vector* cur_box)
{
     ...
     for (int i = 0; i != cur_box->size(cur_box); i += 2)
     {
...
void piece_bomb(piece* this, int x, int y, int size)
{
     ...
     for (int j = 0; j != len; j += 2)
     {
          int p = radius[j];
          int q = radius[j+1];
          int intq = (q <= wthm && q > modw);
          int intp = (p > toph && p <= lower);
          if ( intp && intq )
          {
     ...
     wait(&delay);
     printf("%s\n", empty);
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(empty, buffer);
} 
...
void piece_clear_row(piece* this)
{
     ...
     this->line = 0;
     this->loop(this, piece_check, piece_shift_piece, 1);
     if ( ! this->line ) return;
     ...
     this->loop(this, piece_update, piece_shift_piece, 0);
}        
...
{
    int len = locus.size(&locus);
    for (int i = 0; i != len; i += 2)
    { 
         int x = locus.at(&locus, i)+dx;
         int y = locus.at(&locus, i+1)+dy;
         int intx = (x <= toph || x > lower);
         int inty = (y >= wthm+2 || y <= modw+1);
...
void piece_delete(piece* this)
{
     for (int i = 0; i != height; ++i)
     {
          for (int j = 0; j != width; ++j)
               (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->box_color[i][j]);
          (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->box_map[i], this->box_color[i]);
     } 
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->box_map, this->box_color);

     for (int i = 0; i != prelevel; ++i)
     {
          (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->old_preview_block[i], this->next_preview_color[i]);
     }

     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->old_preview_block, this->next_preview_color);
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->cur_color, this->cur_shadow, this->old_shadow, this->cur_preview_block);

     for (int i = 0; i != color_len; ++i)
          (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->color[i]);
     for (int i = 0; i != prelevel; ++i)
          this->next_preview_piece[i].delete(&this->next_preview_piece[i]);
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(this->color, this->next_preview_piece);
...
void board_matrix()
{
     ...
     int len_str9 = 3*len_str0+2*len_equ+25;
     ...
     int len_str10 = 4*len_str0+len_equ+28;
     ...
     int len_str11 = 5*len_str0+3*len_equ+32;
     ...
     int len_str12 = 4*len_str0+2*len_equ+31;
     ...
     int len_str13 = 4*len_str0+3*len_equ+28;
     ...
     (...) (type, func, ...) {                          \(void, free, __VA_ARGS__);(str, str0, str1, str2, str3, str4, str6, str7, str8, str9, str10, str11, str12, str13, str14, str15);
}
